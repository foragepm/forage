...
async function ipfsAdd(data, hashAlg = 'sha2-256') {
  try{
    var res = await ipfs.add(data, {chunker: 'size-1048576', rawLeaves: true, hashAlg: hashAlg, cidVersion: 1})
...
async function dialPeers(db, topic) {
  listPeers(db).then(async function(peers) {
    ...
    console.log(`${peerIds.length} peer${peerIds.length === 1 ? "" : "s"} online`)
  })
}
...
async function activePeers(topic = 'forest') {
