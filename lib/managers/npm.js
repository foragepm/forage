...
async function importPackage(db, manager, name, version, url) {
  ...
  if(possibleCID){
    ...
    var file = await core.ipfsCat(possibleCID)
...
async function serverHandler(db, req, res) {
  ...
  if(req.headers['user-agent'].match(/npm/)){
    var tarball = await returnTarballEarly(req.url)
    ...
    if(req.method == 'GET' && tarball.name && tarball.cid) {
      ...
      core.tarballHandler(tarball.name+'.tgz', 'application/gzip', tarball.cid, req, res)
    } else if (req.method == 'GET' && metadataName){
      console.log('npm metadata', metadataName)
      metadataHandler(db, metadataName, req, res)
    } else {
      proxy.web(req, res, {
  ...
  proxy.on('proxyRes', function (proxyRes, req, res) {
    ...
    if({name, version} = isTarballRequest(path)) {
      importPackage(db, 'npm', name, version, req.url)
    }
  });
}
