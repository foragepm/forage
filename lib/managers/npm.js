...
async function serverHandler(db, req, res) {
  ...
  proxy.on('proxyRes', async function (proxyRes, req, res) {
    if({name, version} = isTarballRequest(req.url)) {
      var url = 'https://registry.npmjs.org' + req.url
...
function setConfig() {
  require('child_process').exec("npm config set registry=http://0.0.0.0:8005/")
}

function unsetConfig() {
  require('child_process').exec("npm config delete registry")
}
