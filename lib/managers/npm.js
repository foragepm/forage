...
async function watchImporter(db, change) {
  try{
    ...
    if(exists) {
      ...
      core.addUrltoIPFS(db, 'npm', name, versionNumber, version.dist.tarball)
      core.loadMetadata(db, 'npm', name)
    }
  } catch(e){
    // invalid
    console.error(e)
...
async function downloadPackageFromRegistry(db, manager, name, version){
  var metadata = await core.loadMetadata(db, manager, name)
  if(!metadata) {
    console.log("Failed to load metadata for", name)
    return false
  ...
  if(versionData == null){
    console.log('Reloading metadata for', name, version)
    const metadata = await core.loadMetadataFromRegistry(db, manager, name)
    var versionData = metadata.versions[version]
  ...
  return await core.addUrltoIPFS(db, manager, name, version, url)
}
