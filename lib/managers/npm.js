...
async function packageAsJson(db, name) {
  ...
  var versionNumbers = Object.keys(meta.versions)
...
async function updatePackage(db, name) {
  ...
  if (update) {
    ...
    await importPackage(db, name, newestVersion, url, wantedCid)
    return true
  } else {
    return false
  }
...
async function updateMetadata(db, name) {
  try {
    var existingJson = await db.get(`response:npm:versions:${name}`)
  } catch(e) {
    var existingJson = ''
  }
