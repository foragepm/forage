...
async function importPackage(db, name, version, url, wantedCid) {
  ...
  if(possibleCID){
    ...
    if(file){
      debug('Loaded', name, version, 'from IPFS')
...
async function verify(db, name, version, cid) {
  ...
  if (versionData) {
    ...
    if (res){
      debug('Downloaded', name, version, 'from IPFS')
  ...
  if(change.doc.name){
    try{
      ...
      if(exists) {
        ...
        debug('New release:', name, versionNumber)
...
async function loadMetadata(db, name) {
  ...
  if(json){
    ...
    debug('Loading metadata for', name, 'from registry')
