...
async function updatePackage(db, name) {
  ...
  if (update) {
    ...
    var url = update['versions'][newestVersion].dist.tarball
    var wantedCid = await core.announceWant(db, 'npm', name, newestVersion, url)
    await importPackage(db, name, newestVersion, wantedCid)
    return true
  } else {
    return false
  }
...
async function loadMetadata(db, name) {
  ...
  if(json){
    log.info('Loading metadata for', name, 'from cache')
    return JSON.parse(json)
  } else {
    log.info('Loading metadata for', name, 'from registry')
...
async function loadMetadataFromRegistry(db, name) {
  try{
    ...
    log.info("loadMetadataFromRegistry error", name, e)
    return false
  }
}
...
async function updateMetadata(db, name) {
  try{
    ...
    return log.info("fetchMetadata error", name, e)
  }
}
