...
async function importPackage(db, name, version, url) {
  ...
  if(file){
    debug('Metafile loaded from IPFS')
    // guessed CID was correct and loaded from ipfs
    var metafile = file

    cid = await recreateZip(metafile)
    loaded = true
  }

  if(!loaded){
    debug('Failed to load', name, version, 'from IPFS')
    debug('Loading', name, version, 'from', url)
...
async function verify(db, name, version, cid) {
  ...
  if(!checksum){
    debug('Unknown version', 'go', name, version)
    return false
  }
  ...
  if(checksum === integrity){
    ...
    debug('Failed to download', 'go', name, version, 'from IPFS', cid)
    return false
  }
}
...
async function watchImporter(db, change) {
  ...
  if(exists) {
    ...
    debug('New release:', 'go', name, version)
...
async function checkPackages(callback) {
  try{
    ...
    debug("Failed to connect to the go changes feed")
  }
}
