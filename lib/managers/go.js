...
async function versionAsJson(db, name, version) {
  ...
  try{
    var infoCID = await db.get(`info:go:${name}:${version}`)
  } catch {
    var infoCID = null
  }

  try{
    var modCID = await db.get(`mod:go:${name}:${version}`)
  } catch {
    var modCID = null
  }

  try{
    var sumCID = await db.get(`sum:go:${name}:${version}`)
  } catch {
    var sumCID = null
  }
  ...
  return {
    ...
    responses: [
      ...
      {
        url: `https://sum.golang.org/lookup/${escape(name)}@${version}`,
        body: sumCID
      }
    ]
  }
...
async function fetchVersionsList(db, name, force = false) {
  ...
  return await core.fetchResponse(db, `pkg:go:${name}`, url, force)
}
...
async function loadMod(db, name, version) {
  ...
  return await core.fetchResponse(db, `mod:go:${name}:${version}`, url)
}

async function fetchLatest(db, name, force = false) {
  var url = `https://proxy.golang.org/${escape(name)}/@latest`
  var body = await core.fetchResponse(db, `latest:go:${name}`, url, force)
  if(body){
...
async function loadInfo(db, name, version) {
  ...
  return await core.fetchResponse(db, `info:go:${name}:${version}`, url)
}
...
async function fetchChecksum(db, name, version) {
  ...
  var body = await core.fetchResponse(db, `sum:go:${name}:${version}`, url)
  if(body) {
    return body.split("\n")[1].split(' ')[2].split(':')[1]
  } else {
    log.error('Failed to download checksum for', name, version, e)
    return false
  }
}
