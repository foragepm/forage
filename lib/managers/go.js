...
async function importMetadata(db, name, metadata) {
  ...
  for (const [key, value] of Object.entries(metadata.responses)) {
    if(value.body){
      var dbkey = `response:go:${key}:${name}`
  ...
  for (const [number, version] of Object.entries(metadata.versions)) {
    // import cid
    if(version.cid){
      var dbkey = `cid:go:${name}:${number}`
      keys.push([dbkey, version.cid])
      await db.put(dbkey, version.cid)
    }

    // import responses
    for (const [key, value] of Object.entries(version.responses)) {
      if(value.body){
        var dbkey = `response:go:${key}:${name}:${number}`
        keys.push([dbkey, value.body])
        await db.put(dbkey, value.body)
      }
    }
...
async function packageAsJson(db, name) {
  ...
  var versionNumbers = versions.trim().split('\n').sort(function (a, b) {
  ...
  var json = {
    ...
    name: name,
    publicUrl: `https://pkg.go.dev/${name}`,
...
function setConfig(port) {
  require('child_process').exec(`export GOPROXY=http://localhost:${port}`)
}
...
async function updatePackage(db, name) {
  ...
  if (match){
    log.info('No new versions of', 'go', name)
    return
  } else {
    log.info('Found new versions of', 'go', name)
    var version = await getLatestVersion(db, name)
    await downloadVersion(db, name, version)
    return true
  }
}
...
async function serverHandler(db, req, res) {
  ...
  if (/@v\/(.+)\.zip$/.test(path)){
    ...
    if(tarball.cid){
      ...
      var cid = await downloadVersion(db, name, version)
    }
...
async function installCommand(port){
  var env = {
    GOPROXY: `http://localhost:${port}`,
  }
  console.log('Running "go get" ...')
  await execa.command('go get', {env: env});
  return
}
