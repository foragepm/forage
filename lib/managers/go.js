...
function parseGoSum(filepath) {
  ...
  gosum.forEach(function(str) {
    if(str.length > 0){
      var parts = str.split(' ')
      var name = lowercase(parts[0])
      var version = lowercase(parts[1].split('/')[0])
...
function lowercase(str) {
  ...
  for(var i=0; i<str.length; i++) {
    if (str.charCodeAt(i) >= 65 && str.charCodeAt(i) <= 90) {
      newStr += `%21${str.charAt(i).toLowerCase()}`;
    } else {
      newStr += str[i];
    }
  }
...
function unlowercase(string) {
  // TODO replace %21${lowercase} letters with upper case
}
