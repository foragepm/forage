...
notifier.on('have', (data) => {
  core.recordHave(db, data.peerId, data.package)
})

notifier.on('want', (data) => {
  core.respondToWant(db, data.peerId, data.package)
})

notifier.on('republish', (data) => {
  respondToRepublish(db, data.peerId, data.package)
})
...
async function defaultAnnounceCb(msg) {
  ...
  if(action == 'republish'){
    notifier.emit('republish', {peerID: msg.from, package: package})
  }
}
...
async function seed(msg){
  ...
  if(action == 'republish'){
    ...
    return await importPackage(package.manager, package.name, package.version, package.url) // TODO optional cid (package.cid)
  } else {
    return false
  }
}
