...
notifier.on('imported', async (data) => {
  await announceHave(data.manager, data.name)
})

notifier.on('have', async (data) => {
  await core.recordHave(db, data.peerId, data.package) // TODO package may be not be in right format here
  return await respondToRepublish(db, data.peerId, data.package) // TODO package may be not be in right format here
})
...
async function defaultAnnounceCb(msg) {
  ...
  if(action == 'want'){
    // TODO check to see if  include trusted public keys
  ...
  if(action == 'have'){
    ...
    if(trusted){
      var package = signing.parseJsonPayload(signature.payload)

      notifier.emit('have', {peerID: msg.from, package: package})
    }
  }
  return action
}
...
async function respondToRepublish(db, peerId, package) {
  try {
...
async function respondToWant(peerId, package) {
  try {
    var cid = await db.get(`cid:${package.manager}:${package.name}:${package.version}`)
    if(cid){ await announceHave(package.manager, package.name) }
  } catch (e) {
    return false
  }
}

async function announceHave(manager, name) {
  log.info("pubsub 'have':", manager, name)
