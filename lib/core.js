...
async function saveVersion(db, manager, name, version, url, cid) {
  ...
  try {
    debug("pubsub 'republish':", manager, name, version)
...
async function fetchWithTimeout(url, timeout = 10000) {
  ...
  if(res.ok){
    ...
    return false
    debug('fetch error', res.status, res.statusText, `(${url})`)
  }
}

async function attemptIPFSLoad(cid, tmout = 1000){
  try{
...
async function announceWant(db, manager, name, version, url) {
  try{
    ...
    if(cid) {
      debug("existing have for", manager, name, version)
      return cid
    }
  } catch {}

  debug("pubsub 'want':", manager, name, version)
  await ipfs.pubsub.publish('forage', JSON.stringify({
    action: 'want',
    forage: forageVersion(),
    package: {
...
async function announceHave(db, manager, name, version, cid) {
  debug("pubsub 'have':", manager, name, version, cid)
  await ipfs.pubsub.publish('forage', JSON.stringify({
    action: 'have',
    forage: forageVersion(),
    package: {
