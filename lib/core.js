...
notifier.setMaxListeners(concurrency+20)
...
async function saveCid(db, manager, name, version, cid) {
  ...
  try {
    notifier.emit('imported', {manager: manager, name: name, version: version, cid: cid})
  } catch(e) {
...
async function loadPkgKeys(db) {
  return new Promise((resolve, reject) => {
    ...
    db.createReadStream({gte: key, lt: key+'~'})
      .on('data', function (data) {
        var parts = data.key.split(':')
        keys.push(`${parts[1]}:${parts[2]}:${parts[3]}`)
      })
      .on('end', function () {
        resolve(keys)
      })
...
async function announceWant(db, manager, name, version, url) {
  try{
    ...
    if(cid) {
      log.info("Trying existing 'have' for", manager, name, version)
      return cid
    }
  } catch {}
  ...
  try{
    var msg = await pEvent(notifier, 'have', {timeout: 2000, filter: function(msg) {
...
async function lookup(keys){
  ...
  var res = await fetch(`https://index.foragepm.com/packages/lookup`, {
  	method: 'post',
  	body: params
  })
  ...
  if(res.ok){
    ...
    log.error('lookup error', res.status, res.statusText)
    return false
  }
}
