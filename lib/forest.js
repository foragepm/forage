...
async function defaultAnnounceCb(msg) {
  ...
  if (exists) {
    ...
    try {
      cid = await db.get(`cid:${name}:${version}`)
    } catch (e) {
      cid = false
    }
...
async function returnTarballEarly(path) {
  ...
  try {
    var cid = await db.get(`cid:${name}:${version}`)
  } catch (e) {
    var cid = false
  }
...
async function addUrltoIPFS(key, url){
  const {name, version} = splitKey(key)
  try {
    exists = await db.get(`cid:${name}:${version}`)
  } catch (e) {
    exists = false
  }
  ...
  try {
    for await (const file of ipfs.addAll(urlSource(url))) {
      ...
      await db.put(`cid:${name}:${version}`, file.cid.toString())
...
function listPackages() {
  var packages = new Promise((resolve, reject) => {
    var keys = []
    db.createKeyStream({gte: 'cid:', lt: 'cid:~'})
      .on('data', function (data) {
        var parts = data.split(':')
        keys.push(parts[1] + '@' + parts[2])
      })
      .on('end', function () {
        resolve(keys.sort())
      })
...
function listPackageNames() {
  return new Promise((resolve, reject) => {
    var keys = []
    db.createKeyStream({gte: 'pkg:', lt: 'pkg:~'})
      .on('data', function (data) {
        var parts = data.split(':')
        keys.push(parts[1])
      })
...
async function downloadPackageFromIPFS(name, version, cid) {
  try {
    existing_cid = await db.get(`cid:${name}:${version}`)
  } catch (e) {
    existing_cid = false
  }
  ...
  if (versionData) {
    ...
    if (res){
      ...
      await db.put(`cid:${name}:${version}`, cid)
...
async function loadMetadataFromRegistry(name) {
  try{
    ...
    await db.put(`pkg:${name}`, JSON.stringify(json))
    return json
  } catch(e) {
...
async function updateMetadata(name) {
  try{
    ...
    try {
      var existingJson = await db.get(`pkg:${name}`)
    } catch(e) {
      var existingJson = ''
    }
...
async function validate(name, version) {
  ...
  try{
    const cid = await db.get(`cid:${name}:${version}`)
...
async function watchImporter(change) {
  ...
  var key = name+'@'+versionNumber

  try {
    exists = await db.get(`cid:${name}:${versionNumber}`)
  } catch (e) {
    exists = false
  }
...
async function watchKnown() {
  ...
  setupWatcher(async function(change) {
    ...
    if(name){
      try {
        var known = await db.get(`pkg:${name}`)
        if(known){
