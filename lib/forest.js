...
async function subscribePackageAnnoucements(receiveMsg = defaultAnnounceCb) {
  try {
    ...
    await core.savePeers(packageAnnoucementsTopic)
  } catch(e) {
...
async function importLatest(manager, name) {
  if(manager === 'npm'){
    return await npm.importLatest(db, name)
  } else if (manager === 'go'){
    return await go.importLatest(db, name)
  } else {
...
async function importPackage(manager, name, version, url) {
  if(manager === 'npm'){
    return await npm.importPackage(db, name, version, url)
  } else if (manager === 'go'){
    return await go.importPackage(db, name, version, url)
  } else {
    console.log('Unknown manager:', manager)
  }
}
...
async function update(manager, name) {
  if (manager === 'npm'){
    ...
    if (update) {
      ...
      await importPackage('npm', name, newestVersion, update['versions'][newestVersion].dist.tarball)
      return true
    } else {
      return
    }
    ...
    if (match){
      ...
      var version = await go.getLatestVersion(db, name)
