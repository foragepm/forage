...
async function returnTarballEarly(path) {
  var name = isTarballRequest(path)
...
async function addUrltoIPFS(key, url){
  ...
  try {
    for await (const file of ipfs.addAll(urlSource(url))) {
      ...
      await db.put(key, file.cid.toString())
      ...
      await setTarballSize(name, version)
...
async function setTarballSize(name, version) {
  var cid = await db.get(name + "@" + version)
...
async function watchImporter(change) {
  ...
  var key = change.doc.name+'@'+versionNumber
