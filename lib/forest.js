...
async function watchImporter(change) {
  let latest = Object.entries(change.doc.time).filter(([key, value]) => ["created", "modified"].indexOf(key) < 0).sort(function(a, b) { return new Date(b[1]) - new Date(a[1]) })[0]
  let versionNumber = latest[0]
