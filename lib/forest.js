...
async function downloadPackageFromRegistry(name, version){
  ...
  if(verionData){
    ...
    console.log('Reloading data for', name, version)
    const metadata = await loadMetadataFromRegistry(name)
    let verionData = metadata.versions[version]
...
async function updateMetadata(name) {
  try{
    const json = await fetchMetadata(name);
    const existingJson = await db.get(name)
...
async function validate(name, version) {
  ...
  if(verionData && verionData.dist.integrity){
    var integrity = verionData.dist.integrity
  } else {
    var integrity = ssri.fromHex(verionData.dist.shasum, 'sha1').toString()
  }
